...
public class CalabashBaseTest {
	...
	public CalabashBaseTest() {
		this.capabilities = CalabashCapabilities.android("greenhouse-android",
				"4.0.3", "de_DE", "emulator-5554", "emulator",
				"com.springsource.greenhouse");
	}

	@BeforeMethod
	protected void startCalabashDriverServer() throws Exception {
		CalabashNodeConfiguration config = new CalabashLocalNodeConfiguration(
				capabilities, host, port);
		calabashDriverServer = new CalabashAndroidServer();
		System.out.println("starting local driver");
		new Thread(new DriverServerRunnable(calabashDriverServer, config))
				.run();
		// Thread.sleep(50000);
	...
	public class DriverServerRunnable implements Runnable {
		...
		DriverServerRunnable(CalabashAndroidServer server,
		...
		public void run() {
			try {
				...
				e.printStackTrace();
				Assert.fail(
						"The local calabash-driver server cannot be started: ",
						e);
			}
		}
	}
	...
	public class CalabashLocalNodeConfiguration extends
			CalabashNodeConfiguration {
		public CalabashLocalNodeConfiguration(CalabashCapabilities capability,
